...
- NeRF is short for Neural Radiance Fields, which is applied to reconstruct building-scale and even city-scale scenes. This paper proposes **Switch-NeRF** to handle the generalization, poor-learnable and inconsistency problems. Sparsely Gated Mixture of Experts(MoE) is applied tooptimize the the NeRF sub-networks.
