...
The process of generating heatmap is the upsampling. The upsampling process is to map the input C x H x W to the feature matrix in (H' x W')x(C x k x k), H' = $\frac{H-k+2pad}{stride}$+1, W' = $\frac{W-k+2pad}{stride}$+1. Up-sampling is to retain the original image to do the segmentation. Besides, skip architecture is adopted to optimize the output. It upsamples different pooling layers and combine them to optimize the output. The samples of 32x, 16x and 8x are shown below.
