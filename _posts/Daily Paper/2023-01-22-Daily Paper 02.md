...
- This paper proposes **Parametric Rectified Linear Unit(PReLU)** which is the upgraded version of ReLu and LReLu. The definition of PReLU is **f($y_i$) = $y_i$, if $y_i$ > 0; $a_i$$y_i$, if $y_i$ $\le$ 0**. The comparision between ReLU and PReLU is shown below.

- Only a very small number of parameters are added, which means that the computational effort of the network and the risk of over-fitting are increased only a little
