...
- This proposes **Region with CNN Features(R-CNN)**
...
The similarity between R-CNN and OverFeat:
Replace region proposal with sliding windows
The speed is slow because of the shared computation in OverFear

The R-CNN using BB(Replace DPM with CNN): Bounding Box Regression
Proposal CNN_Feature_Extractor SVM BB_Regression
Input images must be the same size(eg. 224 224)
...
5. Identify about 1000-2000 candidate frames in the image (using Selective Search)
6. Scale the image blocks in each candidate frame to the same size and input them into the CNN for feature extraction
7. Use a classifier to determine whether the features extracted from the candidate frames belong to a specific class
8. For the candidate frames belonging to a particular class, the regressor is used to further adjust their positions
