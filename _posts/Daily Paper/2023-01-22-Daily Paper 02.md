...
- This paper proposes **YOLO v3** which is the improved version of YOLO v2. YOLO v3 mainly improves YOLO v2 in three aspects. Firstly, YOLOv3 uses logistic classifiers to predict an objectness score for each bounding box, based on the overlap between the predicted box and the object. If a box has a higher overlap than all other boxes, its score is 1, ignoring those boxes that are not the best and have an overlap greater than a certain threshold (0.5). Secondly, YOLO v3 uses the multi-scale predictions. Thirdly, feature extractor of YOLO v3 adopts Darknet-53 which is shown below.

![Darknet-53](/images/DailyPaper/02/94.png "Darknet-53")

Improved version of YOLO v2
Use residual as the feature extractor
Use FPN to multi-scale detect

No pooling and fully connection
Absorb ResNet DenseNet FPN
