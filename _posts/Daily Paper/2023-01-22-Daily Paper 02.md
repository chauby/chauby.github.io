...
![A Squeeze-and-Excitation Block](/images/DailyPaper/02/71.jpeg "A Squeeze-and-Excitation Block")

![The Comparison between Inception and SE-Inception/ Residual and SE-ResNet](/images/DailyPaper/02/72.jpeg "The Comparison between Inception and SE-Inception/ Residual and SE-ResNet")
...
SENet
Squeeze(Global Average Pooling) and Excitation
SENet strengthens the features of important channels and weakens the features of non-important channels
...
The  upgraded version of DenseNet
Optimize the Gradient Back-Propagation Path
...
EfficientNet
To enlarge the neural network with increase the width height depth by the ratio simultaneously 
...
- This paper also proposes one theorem and two propositions: *Theorem 1. The global minimum of the virtual training criterion C(G) is achieved if and only if $p_g$ = $p_data$. At that point, C(G) achieves the value − log 4.*;
*Proposition 1. For G fixed, the optimal discriminator D is $D_G^*(x) = \frac{p_{data}(x)}{p_{data}(x) + p_g(x)}$*;
*Proposition 2. If G and D have enough capacity, and at each step of Algorithm 1, the discriminator is allowed to reach its optimum given G, and pg is updated so as to improve the criterion: $E_{x~p_{data}}$[log $D^*_G$(x)] + $E_{x~p_g}$[log(1 − $D^*_G$(x))], then $p_g$ converges to p_{data}.*
