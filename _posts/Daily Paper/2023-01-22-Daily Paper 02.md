...
Remove the process of generating proposals
Directly use CNN to detect
1. Multiple scale feature maps
2. Use convolution to detect the result

3. Prior boxes to set the anchors (c + 1 Confidence)
4. Localization loss+confidence loss
5. Data augmentation (Horizontal flip, random crop, color distortion, randomly sample a patch)
Drawbacks: rely on experience, not very good on small objects

Reasons for the faster SSD speed
(1) Reason 1: Firstly, SSD is a one-stage network, which only needs one stage to output results; whereas Faster R-CNN is a two-stage network. Although the bounding boxes of Faster R-CNN are much less, it needs a lot of forward and backward inference (training phase), and it needs to train both networks alternately.
(2) Reason 2: Faster R-CNN requires training not only RPN but also Fast R-CNN, while SSD is actually equivalent to an optimized RPN network, which does not require the later detection and takes a lot of time for forward inference alone.
(3) Reason 3: Although YOLO networks look simpler than SSD networks, YOLO networks contain a large number of fully connected layers. Compared with the FC layer, the CONV layer has fewer parameters; meanwhile, the operation of YOLO to obtain candidate bounding boxes is time-consuming; in the SSD algorithm, the architecture of the VGG network is adjusted and the FC layer is replaced by the CONV layer, which will greatly improve the speed.
(4) Reason 4: Atrous algorithm is used, which can speed up 20%.
(5) Reason 5: SSD sets the input image size, it will crop different size images to 300x300, or 512x512, compared to Faster-rcnn, there will be much less computation on the input.

Translated with www.DeepL.com/Translator (free version)

SSD 速度更快的原因的原因
（1）原因 1：首先 SSD 是一个 one-stage 网络，只需要一个阶段就可以输出结果；而 Faster R-CNN 是一个 two-stage 网络。尽管 Faster R-CNN 的 bounding boxes 少很多，但是其需要大量的前向和反向推理（训练阶段），而且需要交替的训练两个网络；
（2）原因 2：Faster R-CNN 中不仅需要训练 RPN，而且需要训练 Fast R-CNN，而 SSD 其实相当于一个优化了的 RPN 网络，不需要进行后面的检测，仅仅前向推理就会花费很多时间；
（3）原因 3：YOLO 网络虽然比 SSD 网络看起来简单，但是 YOLO 网络中含有大量的全连接层。和 FC 层相比，CONV 层具有更少的参数；同时 YOLO 获得候选 bounding boxes 的操作比较费时；SSD 算法中，调整了 VGG 网络的架构，将其中的 FC 层替换为 CONV 层，这一点会大大的提升速度。
（4）原因 4：使用了 Atrous 算法，该算法能够提速 20%。
（5）原因 5：SSD 设置了输入图片的大小，它会将不同大小的图片裁剪为 300x300，或者 512x512，和 Faster-rcnn 相比，在输入上就会少很多的计算。
